...
Source: https://hackathon-armasuisse.github.io/tracks/track-3/submitting-application/
...
- Keep the data in **`./corpus`** on the VM, which compose mounts read-only at the same path inside the container, with the layout given in [Building your Application](./building-your-application.md).
...
- responses follow the I/O contract in [Building your Application](./building-your-application.md);
...
In that Thursday session you may add guardrails and redeploy. Tag the commit you finish with as `final` and send us the new hash; the endpoint URL stays the same.
